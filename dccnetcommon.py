...
# Buffering up to 2 frames is enough to find misaligned frames
BUF_SIZE: int = (
    MAX_FRAME_SIZE * 2
)
...
def receiveAndCheckFrame(
    sock: socket.socket, skipDataQueue: bool = False
) -> dict[str, Any] | None:
    ...
    while True:
        ...
        if frameStartIdx == -1:
            logging.warning(
                f"receiveAndCheckFrame: Could not sync to received data. SYNC_HEX is missing/corrupted."
            )
            return None
        ...
        if frame["flag"] != "ACK":
            if (
                frame["id"] == dccnet.lastReceivedID
                and frame["checksumHex"] == dccnet.lastReceivedChksumHex
            ):
                logging.warning(
                    f"receiveAndCheckFrame: Received duplicate data frame. Re-sending ACK and skipping..."
                )
            ...
            # This way we can receive frames while waiting to send another
        ...
        if frame["flag"] == "ACK" and not dccnet.waitingForACK:
            if frame["id"] == dccnet.lastTransmitID:
                logging.warning(
                    f"receiveAndCheckFrame: Received duplicate ACK frame. Skipping..."
                )
                continue
...
def sendFrameAndWaitForACK(sock: socket.socket, frame: bytes):
    ...
    if not valid:
        logging.error(
            f"sendFrameAndWaitForACK: Attempt to send invalid frame. Aborting..."
        )
    ...
    while attempts:
        ...
        if frameRecv is not None:
            ...
            if frameRecv["flag"] == "ACK" and frameRecv["id"] == frameSend["id"]:
                logging.debug(
                    f"sendFrameAndWaitForACK: Received ACK for ID {frameSend['id']}"
                )
        ...
        # checkFrame returned None or invalid ACK
        attemptNum: int = MIN_RETRANSMISSIONS_RETRIES - attempts + 1
        logging.warning(
            f"sendFrameAndWaitForACK: No ACK received ({attemptNum}/{MIN_RETRANSMISSIONS_RETRIES}). Retransmitting..."
        )
